  ...
  return <div>
    ...
    <div className="ml-15 bg-[#edf6f9] items-center text-[#006d77] rounded-2xl  me-15 p-3 flex justify-between ">
      ...
      <div className="flex gap-3">
        <span className="bg-[#006d77] w-15 flex justify-center p-1 rounded-2xl" onClick={()=>setGridStyle('grid-cols-4')}>
        ...
        <span className="bg-[#006d77] w-15 flex justify-center p-1 rounded-2xl" onClick={()=>setGridStyle('grid-cols-3')}>
        ...
        <span className="bg-[#006d77] w-15 flex justify-center p-1 rounded-2xl" onClick={()=>setGridStyle('grid-cols-2')}>
          <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#ffffff"><path d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h560q33 0 56.5 23.5T840-760v560q0 33-23.5 56.5T760-120H200Zm320-320v240h240v-240H520Zm0-80h240v-240H520v240Zm-80 0v-240H200v240h240Zm0 80H200v240h240v-240Z"/></svg>
        </span>
      </div>
    </div>
    <div  className={`${gridStyle} grid  gap-4 m-5`}>
    {data?.slice(0,16).map((item,i)=>(
      <PatternOne key={i} image={item.image} name={item.name} price={item.price} description={item.description}/>
    ))}
    </div>
  </div>;
}
